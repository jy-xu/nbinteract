...
from ipywidgets import Dropdown, IntSlider, HBox, interactive, IntText, Label
...
def active_learning(root_path=""):
    ...
    savebutton = Button(description='Save', layout=Layout(height='30px', width='80px'))
    savedone = Label(value="")
    ...
    def update_grid_id(*args):
        df_error = df_test_outputs[(df_test_outputs.image_name == images.value)]
        grid_id.value = \
        df_error.grid_id[(df_error.grid_x == slider_x.value) & (df_error.grid_y == slider_y.value)].values[0]
    ...
    def update_errors_value(*args):
        df_error = df_test_outputs[(df_test_outputs.image_name == images.value)]
        df_error_id = \
        df_error.error_id[(df_error.grid_x == slider_x.value) & (df_error.grid_y == slider_y.value)].values[0]
    ...
    def save_to_csv(*args):
        with output3:
            ...
            df_test_outputs.to_csv(root_path + 'test_outputs.csv', index=False)
            savedone.value = "Done!"
            display(savedone)
            time.sleep(1)
            output3.clear_output()
    ...
    savebutton.on_click(save_to_csv)
    ...
    output3 = Output(layout={'width': '50px', 'height': '30px'})
    ...
    display(HBox([images, errors, prevbutton, nextbutton, savebutton, output3], layout=box_layout))
    box_layout = Layout(display='flex', flex_flow='row', justify_content='flex-start', align_items='center')
